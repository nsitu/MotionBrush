...
import * as Magick from './magickApi.js';
    ...
    let DoMagickCall = async function () {
      let fetchedSourceImage = await fetch("sloth-wide.jpg");
...
async function go(){
    ...
    if (paths != null){
        for (const path of paths){
            ...
            // simplify the path by describing it as a set of points coordinates
            ...
            // bake transformations for all paths
            // bake transformations for entire SVG.
            // Snap.path.map(
...
// it needs some work to be functional. 
  function optimizeSegments(segments, path){
   ...
   function findSegment(path, pathLength, startLocation, increment, split = false, level = 1){
        ...
        let offsetB = (increment  / 100) * (config.segmentOverlap / 100) / 2;
        let offsetC = (increment  / 100) * (config.segmentOverlap / 100);
...
function renderLine(line){
    ...
    canvasContext.lineWidth = config.strokeThickness;
